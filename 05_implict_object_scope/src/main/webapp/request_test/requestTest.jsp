...
<body>
	...
	<!-- 
		...
		- 기본값 (application/x-www-form-urlencoded): 전송되는 모든 문자들을 서버로 보내기 전에 인코딩 됨을 명시 
		
		multipart/form-data
		전달되는 데이터에 이진 데이터가 포함되어 있으므로 모든 문자를 인코딩하지 않음을 명시
		이 방식은 문자가 아닌 데이터(파일, 이미지) 등을 서버에 전송할 때 사용한다. 
		파일 업로드
		
		text/plain 
		일반문자 - 공백은 + 기호로 변환하지만 나머지 문자는 인코딩되지 않음을 명시 
		
		
	 -->
	 <!-- 포스트 방식일때만 적용 / get일떄는 url로 적용되서 관련x
	...
	<form action="requestResult.jsp" method="post" >
		...
		취미 : <input type="checkbox" name="hobby" value="축구" /> 축구
			  ...
			  <br/> <input type="submit" /> <!-- submit버튼 value 미지정하면 기본값은 제출 -->
	</form>
</body>
</html>