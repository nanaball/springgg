...
<section class="wrap">
	<table>
		...
		<c:if test="${!empty sessionScope.member }">
			<tr>
				<th colspan="5" style="text-align: right"><a
					href="boardWrite.qna">질문 작성하러 가기</a></th>
			</tr>
		</c:if>
		<tr>
			<th>글번호</th>
				...
				<c:forEach var="board" items="${list}">
					<c:choose>
						<c:when test="${board.qna_delete eq 'N'.charAt(0)}">
							<tr>
								<td>${board.qna_num}</td>
								<td><c:if test="${board.qna_re_lev !=0}">
										<c:forEach begin="1" end="${board.qna_re_lev}">
											&nbsp;&nbsp;&nbsp;
										</c:forEach>
										└> 
									</c:if> <a href="qnaAbout.qna?qna_num=${board.qna_num}"
									title="게시글 상세보기"> ${board.qna_title} </a></td>
