...
public class MemberServiceImpl implements MemberService {
	...
	public void logOut(HttpServletRequest request, HttpServletResponse response) {
		// 로그인된 사용자 정보 - session에서 member 제거
		request.getSession().removeAttribute("member");
		
		// 혹시라도 쿠키가 등록되어 있으면 삭제
	...
	public void findPassSubmit(HttpServletRequest request, HttpServletResponse response) {
		...
		// 데이터베이스 mvc_member 테이블에서 id와 이름이 일치하는 사용자가 존재하는지 체크
		...
		try {
			...
			mail.append("<form action ='http://10.100.205.92:8080");
			...
			mail.append("<input type='submit' name='code' value='이메일인증 완료'>");
			mail.append("</form>");
			mail.append("</body>");
			mail.append("</html>");
			...
			e.printStackTrace();
			out.println("<script>");
			out.println("alert('서비스에 문제가 없습니다. 다시 이용해주세요.\\n"+e.getMessage()+"');");
