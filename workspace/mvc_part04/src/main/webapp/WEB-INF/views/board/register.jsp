...
<!-- 
	action이 생략되면 현재 주소표시줄의 요청 경로로 요청 전달
	board/register 
-->
...
<!-- 글쓰기 에디터 https://www.tiny.cloud/ -->
<!-- Place the first <script> tag in your HTML's <head> -->
<script src="https://cdn.tiny.cloud/1/lf9k3ryzj4kayghsq2pldteiont7sz6y6kyo83nw3d79n879/tinymce/6/tinymce.min.js" referrerpolicy="origin"></script>
...
<script>
	// drag n drop 시 브라우저가 파일을 해석 하려는 기본 이벤트를 제거
...
//				console.log(data);
	...
	// li로 출력에 필요한 파일 정보를 반환할 함수
	function getFileInfo(fullName){
		// 이미지 경로, 원본 파일 이름, 원본 파일 요청 경로
		...
		if(checkImage(fullName)){
			...
			getLink = contextPath+"/displayFile?fileName="+fileName;
		}
	...
	// 첨부파일 삭제
	// 원래 href 버튼 속성은 get 임
	$(".uploadList").on("click", ".delBtn", function(event){
		event.preventDefault();	// a tag 기본 이벤트 처리제거
		// .delBtn인 요소들 중에 click event가 발생한 대상요소
		let target = $(this);
		...
		$.ajax({
			...
			success : function(data){
				alert(data);
				// closest target 기준으로 부모 요소 중에 선택자로 지정된 가장 가까운 부모 요소를 검색
