...
//			headers.setContentType(MediaType.TEXT_PLAIN);
			...
			// 정상적일때 					 (전달할데이터, 헤더정보, 상태(응답)코드)- 데이터와 헤더정보는 생략가능
			entity = new ResponseEntity<>(message, headers, HttpStatus.OK);	
//			entity = new ResponseEntity<>("OK", HttpStatus.OK);		// 발생한 예외 컨트롤러에서 처리
		} catch (Exception e) {
			entity = new ResponseEntity<>(e.getMessage(),HttpStatus.BAD_REQUEST);
		}
		return entity;
		// return ResponseEntity.ok().build();
		// return ResponseEntity.badRequest().build(); 	// 400 오류
	}
	...
	// 정보를 map에 저장하여 반환
	// data : {list : List<CommnetVO>, pm : PageMaker}
